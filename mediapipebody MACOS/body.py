# MediaPipe Body
import mediapipe as mp
from mediapipe.tasks import python
from mediapipe.tasks.python import vision
import numpy as np
import cv2
import threading
import time
import global_vars
import struct
import os
import socket

# Function to list available cameras

import subprocess

def list_camera_devices():
    # Run a command to get the list of available cameras using system_profiler
    result = subprocess.run(['system_profiler', 'SPCameraDataType'], stdout=subprocess.PIPE)
    output = result.stdout.decode()

    # Parse the output to extract camera names
    camera_names = []
    for line in output.splitlines():
        if "Model ID:" in line:  # Cameras usually show up with "Model ID" or similar tags
            camera_name = line.split(":")[1].strip()
            camera_names.append(camera_name)
    
    # If camera_names appear in reverse order, you can reverse them here:
    return camera_names[::-1]  # Reverse the list if needed

# Capture thread captures images from the WebCam on a separate thread (for performance)
class CaptureThread(threading.Thread):
    cap = None
    ret = None
    frame = None
    isRunning = False
    counter = 0
    timer = 0.0
    
    def __init__(self, camera_index):
        super().__init__()
        self.camera_index = camera_index

    def run(self):
        self.cap = cv2.VideoCapture(self.camera_index)
        if global_vars.USE_CUSTOM_CAM_SETTINGS:
            self.cap.set(cv2.CAP_PROP_FPS, global_vars.FPS)
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, global_vars.WIDTH)
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, global_vars.HEIGHT)

        time.sleep(1)
        print("Opened Capture @ %s fps" % str(self.cap.get(cv2.CAP_PROP_FPS)))
        
        while not global_vars.KILL_THREADS:
            self.ret, self.frame = self.cap.read()
            self.isRunning = True
            if global_vars.DEBUG:
                self.counter += 1
                if time.time() - self.timer >= 3:
                    print("Capture FPS: ", self.counter / (time.time() - self.timer))
                    self.counter = 0
                    self.timer = time.time()

# Function to ask user for camera selection
def select_camera():
    camera_list = list_camera_devices()
    
    if not camera_list:
        print("No cameras found!")
        return None
    
    print("Available cameras:")
    for idx, cam in enumerate(camera_list):
        print(f"{idx + 1}. {cam}")
    
    while True:
        try:
            choice = int(input("Type number of camera you want to use: "))
            if 1 <= choice <= len(camera_list):
                return choice - 1  # Return the selected camera index
            else:
                print("Invalid choice. Please select a valid camera number.")
        except ValueError:
            print("Please enter a valid number.")

# Body thread processes captured images and communicates with Unity
class BodyThread(threading.Thread):
    data = ""
    dirty = True
    pipe = None
    timeSinceCheckedConnection = 0
    timeSincePostStatistics = 0

    def compute_real_world_landmarks(self, world_landmarks, image_landmarks, image_shape):
        try:
            frame_height, frame_width, channels = image_shape
            focal_length = frame_width * 0.6
            center = (frame_width / 2, frame_height / 2)
            camera_matrix = np.array(
                [[focal_length, 0, center[0]],
                 [0, focal_length, center[1]],
                 [0, 0, 1]], dtype="double"
            )
            distortion = np.zeros((4, 1))

            success, rotation_vector, translation_vector = cv2.solvePnP(
                objectPoints=world_landmarks,
                imagePoints=image_landmarks,
                cameraMatrix=camera_matrix,
                distCoeffs=distortion,
                flags=cv2.SOLVEPNP_SQPNP
            )
            transformation = np.eye(4)
            transformation[0:3, 3] = translation_vector.squeeze()

            model_points_hom = np.concatenate((world_landmarks, np.ones((33, 1))), axis=1)
            world_points = model_points_hom.dot(np.linalg.inv(transformation).T)

            return world_points
        except AttributeError:
            print("Attribute Error: shouldn't happen frequently")
            return world_landmarks 

    def run(self):
        mp_drawing = mp.solutions.drawing_utils
        mp_pose = mp.solutions.pose
        
        camera_index = select_camera()  # Prompt user to select a camera
        if camera_index is None:
            return  # Exit if no camera is selected

        capture = CaptureThread(camera_index)
        capture.start()

        with mp_pose.Pose(min_detection_confidence=0.80, min_tracking_confidence=0.5, model_complexity=global_vars.MODEL_COMPLEXITY, static_image_mode=False, enable_segmentation=True) as pose:
            while not global_vars.KILL_THREADS and not capture.isRunning:
                print("Waiting for camera and capture thread.")
                time.sleep(0.5)
            print("Beginning capture")
                
            while not global_vars.KILL_THREADS and capture.cap.isOpened():
                ti = time.time()

                # Fetch stuff from the capture thread
                ret = capture.ret
                image = capture.frame

                if image is None:
                    return
                
                #image.flags.writeable = global_vars.DEBUG
                results = pose.process(image)
                tf = time.time()

                # Rendering results
                if global_vars.DEBUG:
                    if time.time() - self.timeSincePostStatistics >= 1:
                        print("Theoretical Maximum FPS: %f" % (1 / (tf - ti)))
                        self.timeSincePostStatistics = time.time()
                        
                    if results.pose_landmarks:
                        mp_drawing.draw_landmarks(image, results.pose_landmarks, mp_pose.POSE_CONNECTIONS, 
                                                   mp_drawing.DrawingSpec(color=(255, 100, 0), thickness=2, circle_radius=4),
                                                   mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=2, circle_radius=2))

                # Socket connection handling and reconnection detection
                if self.pipe is None:
                    try:
                        if os.name != 'nt':  # For non-Windows (macOS, Linux)
                            server_address = '/tmp/UnityMediaPipeBody.sock'
                            if os.path.exists(server_address):
                                os.unlink(server_address)  # Ensure no existing socket with the same name
                            self.pipe = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
                            self.pipe.bind(server_address)
                            self.pipe.listen(1)
                            print("Waiting for Unity to connect via Unix socket...")
                            connection, client_address = self.pipe.accept()  # Wait for Unity to connect
                            print("Connection established.")
                            self.pipe = connection
                        else:
                            # Windows named pipe logic
                            self.pipe = open(r'\\.\pipe\UnityMediaPipeBody', 'r+b', 0)
                        
                    except (FileNotFoundError, socket.error) as e:
                        print(f"Waiting for Unity project to run... {e}")
                        self.pipe = None

                    self.timeSinceCheckedConnection = time.time()

                if self.pipe is not None:
                    # Check if Unity has disconnected
                    try:
                        if results.pose_world_landmarks:
                            image_landmarks = results.pose_landmarks
                            world_landmarks = results.pose_world_landmarks

                            model_points = np.float32([[-l.x, -l.y, -l.z] for l in world_landmarks.landmark])
                            image_points = np.float32([[l.x * image.shape[1], l.y * image.shape[0]] for l in image_landmarks.landmark])
                            
                            body_world_landmarks_world = self.compute_real_world_landmarks(model_points, image_points, image.shape)
                            body_world_landmarks = results.pose_world_landmarks
                            
                            self.data = ""  # Reset data buffer before filling
                            for i in range(33):
                                self.data += "FREE|{}|{}|{}|{}\n".format(i, body_world_landmarks_world[i][0], body_world_landmarks_world[i][1], body_world_landmarks_world[i][2])
                            for i in range(33):
                                self.data += "ANCHORED|{}|{}|{}|{}\n".format(i, -body_world_landmarks.landmark[i].x, -body_world_landmarks.landmark[i].y, -body_world_landmarks.landmark[i].z)

                            # Send data to Unity
                            s = self.data.encode('utf-8')
                            try:
                                self.pipe.sendall(struct.pack('I', len(s)) + s)  # Send size first, then data
                            except Exception as ex:
                                print("Failed to write to pipe. Is the Unity project open?")
                                self.pipe.close()  # Close the pipe if write fails
                                self.pipe = None  # Reset pipe to trigger reconnection
                                
                    except Exception as e:
                        print(f"Error while sending data to Unity: {e}")
                        if self.pipe:
                            self.pipe.close()
                        self.pipe = None  # Reset pipe if there's an error
                        
        if self.pipe is not None:
            self.pipe.close()
        capture.cap.release()

# Initialize and run BodyThread
if __name__ == "__main__":
    body_thread = BodyThread()
    body_thread.start()