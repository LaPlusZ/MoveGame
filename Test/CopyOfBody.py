        ...
        with mp_pose.Pose(min_detection_confidence=0.80, min_tracking_confidence=0.5, model_complexity = global_vars.MODEL_COMPLEXITY,static_image_mode = False,enable_segmentation = True) as pose: 
            ...
            while not global_vars.KILL_THREADS and capture.cap.isOpened():
                ...
                if results.pose_landmarks:
                    ...
                    left_calf = abs(left_ankle.y - left_knee.y)
                    right_calf = abs(right_ankle.y - right_knee.y)
                    left_thigh = abs(left_knee.y - left_hip.y)
                    right_thigh = abs(right_knee.y - right_hip.y)
                    if (left_wrist.y < left_ear.y) or (right_wrist.y<right_ear.y):
                        if( left_thigh <left_calf*0.6)or (right_thigh<right_calf*0.6):
                            # print(position)
                            if position != "running":
                                print("Walk+1") 
                            position = "running"    
